...
#other imports needed if running this .py alone
...
for classifier in [GaussianNB(),BernoulliNB(binarize=.33),SVC(class_weight={1:1/.36,0:1/.64},kernel='linear'),
                   LinearRegression(),KNeighborsClassifier(),DecisionTreeClassifier()]:
    bc_NewModel = classifier

    bc_NewModel.fit(new_train_data.values,train_labels.values.ravel())
    GNBpredictions = bc_NewModel.predict(new_test_data.values)
    if type(classifier) == type(LinearRegression()):
        GNBpredictions = (GNBpredictions>.33)
